...
const representationFromQuestion = (
  ...
  if (q.diagram) {
    if (q.diagram.type === "graph") return "graph";
  ...
  if (
    q.questionFamily === "diagram-analysis"
  ) return "diagram";
  if (
    q.questionFamily === "data-analysis"
  ) return "data";
  if (
    q.questionFamily === "calculation"
  ) return "symbolic";
  return "verbal";
};
...
export function toCAESARQuestion(
  ...
  return {
    ...
    questionFamily:
      q.questionFamily as QuestionFamily,
    type: q.type,
    representationType:
      representationFromQuestion(q),
    verified:
      q.verification?.status === "verified",
    prerequisites:
      q.relationships?.prerequisites ?? [],
    contrastsWith:
      q.relationships?.contrastsWith ?? [],
    relatedConceptIds:
      q.relationships?.related ?? [],
    importance:
      q.tags?.some((tag) =>
    ...
    estimatedTimeMs:
      estimateQuestionTime(q),
  };
}

function estimateQuestionTime(
  q: Question,
): number {
